define(function (require, exports, module) {
    ...
    layer.msg = function (text) {
        layer.open({content: text, time: 2, skin: 'msg'});
    };
...
// lottery: 开奖及下注
    window.lottery = {
        ...
        game_zhui_hao: function () {
            //
            var addRet = lottery.game_add_code(true);
            if (!addRet) {
                return false;
            }
        ...
        set_game_zhui_hao: function () {
            ...
            if (LotteryCart.bets.length === 0) {
                layer.msg('请先选择投注号码');
            } else {
                var choosed_data = LotteryCart.bets[0].code;
                console.log(LotteryCart.bets);

                var beiShu = choosed_data.beiShu;
                var mode = choosed_data.mode;
                var amount = (choosed_data.actionNum * beiShu * mode).toFixed(3);
                $.get('/game/zhuihao?type=' + game.type + '&num=' + num + '&beiShu=' + beiShu + '&mode=' + mode + '&amount=' + amount, function (ret) {
                    console.log(ret);
                    layer.closeAll();
                    layer.open({
                        ...
                        yes: function () {
                            //确认投注
                            lottery.zhuihao_sure();
                        }
                    });
                    $(".choose_all").trigger("click");
                    $(".submit-bets-body").hide();
                }, "JSON");
        ...
        zhuihao_cancel: function () {
            $.dialogue({
                ...
                yes: {
                    text: '确定',
                    func: function () {
                        $('#btnZhuiHao').die('click').removeAttr('style').text('智能追号');
        ...
        set_play_Pl: function (value, flag) {
            ...
            if (value) {
                value.bonusProp = parseFloat(value.bonusProp);
                value.bonusPropBase = parseFloat(value.bonusPropBase);
            ...
            console.log(lottery.play_Pl.value);
            window.fandianSelArr = {};
            var dom = $('#fandian-value');
            var max_userFanDian = dom.attr("data-user-fd");
            var max_fanDian = $('#play-mod b.on').data('maxFanDian');
            var mode = $.cookie('mode') || '2.000';
            mode = parseFloat(mode) / 2


            var fanDian = parseFloat(max_userFanDian);
            var fanDian_this = $.cookie('fandian') || '0.0';
            fanDian_this = parseFloat(fanDian_this).toFixed(1);
            var html = '';
            console.log("FLAG:", flag);

            flag = false;
            if (flag) {
                html += '<div class="text icon-beaker">玩法不支持选择[奖金组-返点]</div>';
            } else {
                ...
                //min奖金组,算小奖金
                var selected_value = {
                    fanDian: max_userFanDian * 1,
                    bonus: value.bonusPropBase,
                    jjz: value.baseJJZ
                };

                window.fandianSelArr[(max_userFanDian * 1).toFixed(1)] = selected_value;
                ...
                //求出递减率
                var tmpCover = ((value.bonusProp - value.bonusPropBase) / max_userFanDian).toFixed(4);
                html += '<option ~selected~ data-fd="' + max_userFanDian + '" value="' + selected_value + '"></option>';
                while (true) {
                    fanDian = (fanDian * 10 - 1) / 10;

                    if (fanDian < 0) {
                        break;
                    }
                    //计算奖金
                    var bonus = value.bonusPropBase + tmpCover * (max_userFanDian - fanDian);

                    var option_value = {fanDian: fanDian, bonus: (Math.floor(bonus * mode * 1000) / 1000).toFixed(4)};
                    window.fandianSelArr[fanDian.toFixed(1)] = option_value;
                    ...
                    html += '<option data-fd="' + fanDian + '" ' + selected_str + ' value="' + option_value + '">奖金:' + bonus + ' 返点:' + fanDian + '%</option>';
                }
                html = html.replace('~selected~', selected ? '' : 'selected');
                html += '</select>';

                //写入0.0
                var default_fanDian = window.fandianSelArr['0.0'];
                console.log(default_fanDian);
                $(".left_fd_label").html(default_fanDian.fanDian.toFixed(1));
                $(".right_jjz_label").html(default_fanDian.bonus);
        ...
        /**
         *  计算已添加预投注的投注数和金额
         *  如果是True是直接更新界面，如果false返回数据
         * @returns {{all_count: number, all_amount: number}}
         */
        calc_amount: function () {
            ...
            //计算购物车数据
            if (LotteryCart.bets.length > 0) {
                LotteryCart.bets.forEach(function (item) {
                    count += item.actionNum;
                    amount += parseFloat(item.amount);
                });
            }
            LotteryCart.total.allAmount = amount;
            LotteryCart.total.allCount = count;
            if (real) {
                $('#all-count').text(count);
                $('#all-amount').text(amount.round(2));
            } else {
                return {all_count: count, all_amount: amount};
            }
        },
        ...
        game_remove_code: function () {
            LotteryCart.clear();
            console.log("Clear投注内容");
            lottery.calc_amount(true);
        },
        ...
        game_add_code: function () {
            ...
            if (window.GAME_TOTAL.stop) {
                layer.msg('本平台已经停止购买');
                return false;
            }
            if (window.GAME_TOTAL.ban) {
                layer.msg('代理不能下注');
                return false;
            }
            ...
            //当前返点
            var currentFanDianValue = $(".slider-bar-fd").attr('data-select-fd');
            console.log(window.fandianSelArr);
            var fandian_value = window.fandianSelArr['0.0'];
            if (!fandian_value) {
                console.log(currentFanDianValue);
                $.error('赔率异常，请稍后尝试');
                return false;
            }
            var fanDian;
        ...
        setAmountTotal: function (all_count, all_amount) {
            $('#all-count').text(all_count);
            $('#all-amount').text(all_amount.round(2));
            LotteryCart.total.allCount = all_count;
            LotteryCart.total.allAmount = all_amount;
        },
        ...
        game_add_code_func: function (code, opt, real) {
            ...
            if ($.isArray(code)) {
                ...
                if (real === false) {
                    ...
                    //更新投注额度
                    lottery.setAmountTotal(all_count, all_amount);
                }
                return;
            }
            ...
            try {
                ...
                //回去玩法
                var playedName = code.playedName || $("#game-play .play-list select[name='play_list_select']").val();
                var weiShu = 0;
                ...
                if ((weizhiTypejsh == 'z3_r3_zuhetouzhu' && caizhongTypejsh == '22')
                    ...
                    for (var iii = 0; iii < weizhiAr.length; iii++) {
                        ...
                        var codeItem = {
                            ...
                            code_list: wei + (code.actionData.length > 18 ? (code.actionData.substr(0, 5) + '...') : code.actionData),
                            actionNum: code.actionNum,
                            amount: amount.round(2),
                            beiShu: code.beiShu,
                            modeName: modeName[code.mode],
                        };
                        LotteryCart.bets.push(codeItem);
                        var tr = $('<tr class="code">').data('code', code)
                            .append($('<td style="display: none;">').append(playedName))
                            .append($('<td class="code-list">').append())
                    ...
                    if (real === true) {
                        ...
                        lottery.calc_amount(true);//true修改文本
                        $.cookie('beiShu', opt.beiShu);
                        $('#textarea-code').val("");
                        $('#zhuiHao').val(0);
                    }
                ...
                if (real === false) {
                    if (recursive === 0) {
                        ...
                        return {all_count: all_count, all_amount: all_amount};
                    }
                } else {
                    lottery.calc_amount(true);
                }
        ...
        bets_fresh: function (callback) {
            $.load('/game/bets?client_type=mobile&id=' + game.type, '#my-bets', {
                show: null,
                callback: function () {
                    $('#bet-cancel').fadeOut();
                },
        ...
        game_post_code: function () {
            ...
            //是否是追号
            var is_zhuihao = arguments[0] === true ? true : false;
            if (!is_zhuihao) {
                var addRet = lottery.game_add_code(true);
                if (!addRet) {
                    return false;
            ...
            bets.forEach(function (item) {
                var length = item.code.actionData.length;
                if (length > 2048) {
                    var actionData = base64.encode(RawDeflate.deflate(item.code.actionData));
                    item.code.actionData = 'deflate-' + length + '-' + actionData;
                }
                code.push(item.code);
            });
            ...
            /*#region同步验证期号  start*/
            var current = JSON.parse($.ajax({
                url: '/game/current?id=' + window.GAME_TOTAL.type,
                type: 'post',
                async: false,
                cache: false,
            }).responseText);
            ...
            html += '<div class="submit-bets-title icon-basket">确定要购买第<span>' + current.actionNo + '</span>期彩票？</div>';
            html += '<div class="submit-bets-info">';
            html += '<div class="count left icon-chart-bar">总投注数：<span class="num">' + LotteryCart.total.allCount + '</span>注</div>';
            html += '<div class="amount left icon-yen">购买金额：<span class="num">' + LotteryCart.total.allAmount + '</span>元</div>';
            html += '</div>';
            html += '</div>';
            ...
            layer.open({
                    ...
                    yes: function (index) {
                        data['type'] = window.GAME_TOTAL.type;
        ...
        countdown: function (diffTime, actionNo) {
            ...
            if (diffTime <= 0) {
                ...
                $dom.html(timeStr_new);
                if (S && h == 0 && m == 5 && s == 0) {
                    voice.play('/static/sound/time.mp3', 'voice-stop-time');
                }
                if (h == 0 && m == 0 && s == 0) {
        ...
        fresh: function () {
            ...
            $("cp_game").load("/game/mb_index?" + url, "POST");
        },
        /**
         * @name 获取本期盈亏
         * @param int type 彩种ID
            ...
            dt_d: function (p) {
                if ($(this).is('.checked')) {
                    ...
                    if (max == 1) {
                        ...
                        if (max == count) {
                            layer.msg('最多只能选择<span class="btn btn-red">' + max + '</span>位胆码');
                        } else {
...
// 投注函数库
        funcs: {
            ...
            tzDesAlgorSelect: function () {
                ...
                code = code.join(',');
                len = lottery.DescartesAlgorithm.apply(null, code.split(",").map(function (v) {
...
// beter: 下注处理
    window.beter = {
        ...
        bet_select: function () {
            ...
            $(document).on('click', '#my-bets td.select', function (event) {
                $('#bet-cancel').fadeIn();
            ...
            $(document).on('click', '#my-bets td.select_all', function () {
            ...
            $(document).on('click', '#my-bets .remove_single', beter.remove_single);
            beter.selected = true;
        },
        // 批量撤单
                ...
                loadpage.before(function () {
                    $.ajax({
                        ...
                        error: function (xhr, textStatus, errThrow) {
                            layer.msg(errThrow || textStatus);
                        },
                        success: function (data, textStatus, xhr) {
                            var error_message = xhr.getResponseHeader('X-Error-Message');
                            if (error_message) {
                                layer.msg(error_message === 'dialogue' ? data : decodeURIComponent(error_message));
                            } else {
                                chekced.each(function () {
        ...
        remove_single: function () {
            ...
            $.post('/bet/remove_single?id=' + id, {}, function (ret) {
                if (ret.code == 200) {
                    var p = $this.parent();
                    if (remove) {
                        p.parent().remove();
