...
(function() {
	...
	if (!ca && !fa) {
		E = {
			...
			pathToVML: function(a) {
				for (var b = a.length, c = []; b--;) if (qa(a[b])) c[b] = w(a[b] * 10) - 5;
				else if (a[b] === "Z") c[b] = "x";
		...
		var Ma = {
			...
			color: function(a, b, c, d) {
				...
				if (i) {
					...
					if (c === "fill") if (i === "gradient") c = n.x1 || n[0] || 0, a = n.y1 || n[1] || 0, l = n.x2 || n[2] || 0, n = n.y2 || n[3] || 0, v = 'angle="' + (90 - V.atan((n - a) / (l - c)) * 180 / ma) + '"', va();
					else {
						var j = n.r,
							s = j * 2,
							x = j * 2,
	...
	ha.prototype = {
		...
		defaultLabelFormatter: function() {
			...
			else if (f && a >= 1E3) for (; f-- && g === x;) c = Math.pow(1E3, f + 1), a >= c && b * 10 % c === 0 && e[f] !== null && (g = B.numberFormat(b / c, -1) + e[f]);
		...
		setTickInterval: function(a) {
			...
			if (u && q(b.dataMin)) if (l = l || 0, !q(A) && b.min < l && b.dataMin >= l) b.min = l;
	...
	ha.prototype.getLogTickPositions = function(a, b, c, d) {
		...
		else if (a >= 0.08) for (var f = T(b), h, i, j, k, m, e = a > 0.3 ? [1, 2, 4] : a > 0.15 ? [1, 2, 4, 6, 8] : [1, 2, 3, 4, 5, 6, 7, 8, 9]; f < c + 1 && !m; f++) {
			i = e.length;
	...
	Va.prototype = {
		...
		inClass: function(a, b) {
			for (var c; a;) {
				if (c = K(a, "class")) if (c.indexOf(b) !== -1) return !0;
	...
	R.prototype = {
			...
			if (d !== !1 && h && g === h && !e.cropped && !e.hasGroupedData && e.visible) o(a, function(a, b) {
				...
				if (l && h > l) {
					...
					if (qa(k)) {
						...
						e.xIncrement = n
					} else if (Ga(k)) if (s) for (c = 0; c < h; c++) k = a[c], u[c] = k[0], r[c] = k.slice(1, s + 1);
					else for (c = 0; c < h; c++) k = a[c], u[c] = k[0], r[c] = k[1];
					else la(12)
				} else for (c = 0; c < h; c++) if (a[c] !== x && (k = {
					series: e
		...
		processData: function(a) {
			...
			if (m && this.sorted && !k && (!i || d > i || this.forceCrop)) if (b[d - 1] < n || b[0] > l) b = [], c = [];
		...
		getExtremes: function(a) {
			...
			d = a.length;
			for (m = 0; m < d; m++) if (j = c[m], k = a[m], i = k !== null && k !== x && (!b.isLog || k.length || k > 0), j = this.getExtremesFromAll || this.options.getExtremesFromAll || this.cropped || (c[m + 1] || j) >= g && (c[m - 1] || j) <= h, i && j) if (i = k.length) for (; i--;) k[i] !== null && (e[f++] = k[i]);
			else e[f++] = k;
			this.dataMin = Oa(e);
			this.dataMax = Da(e)
		},
		...
		drawPoints: function() {
			...
			if (m.enabled !== !1 || this._hasPointMarkers) for (f = b.length; f--;) if (g = b[f], d = T(g.plotX), e = g.plotY, k = g.graphic, l = g.marker || {}, o = !! g.marker, a = q && l.enabled === x || l.enabled, r = g.isInside, a && e !== x && !isNaN(e) && g.y !== null) if (a = g.pointAttr[g.selected ? "select" : ""] || n, h = a.r, i = p(l.symbol, this.symbol), j = i.indexOf("url") === 0, k) k[r ? "show" : "hide"](!0).animate(t({
				x: d - h,
				y: e - h
			}, k.symbolName ? {
				width: 2 * h,
		...
		getAttribs: function() {
			...
			if (!k || g < k || j) for (; g--;) {
				...
				if (k.options) for (A in n) q(c[n[A]]) && (j = !0);
				if (j) {
					c = c || {};
					j = [];
					d = c.states || {};
		...
		getSegments: function() {
			...
			if (this.options.stacking && !this.cropped) {
				...
				o(d, function(b) {
					var d = null,
						k;
					if (!m || h[b] && h[b].y !== null) if (h[b]) c.push(h[b]);
					else {
				...
				if (L) {
					...
					if (n > 0) {
						...
						for (H = 0; H < L; H++) {
							...
							if (K < H && J[H] !== null) K = H;
							else for (v < L - H + K && J[H] !== null && (K = v - L + H); J[K] === null;) K++;
							N.push({
								i: K,
								y: J[K]
							});
	...
	(function(a) {
		...
		b.prototype.hideOverlappingLabels = function(a) {
			...
			for (e = 0; e < b; e++) {
				j = a[e];
				for (d = e + 1; d < b; ++d) if (k = a[d], j && k && j.placed && k.placed && j.newOpacity !== 0 && k.newOpacity !== 0 && (m = j.alignAttr, n = k.alignAttr, l = 2 * (j.box ? 0 : j.padding), m = !(n.x > m.x + (j.width - l) || n.x + (k.width - l) < m.x || n.y > m.y + (j.height - l) || n.y + (k.height - l) < m.y)))(j.labelrank < k.labelrank ? j : k).newOpacity = 0
			}
			c(a, function(a) {
				var b, c;
				if (a) {
