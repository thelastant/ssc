...
$(function () {
    ...
    var zhuihao_data_func = function () {
        ...
        $('.zhuihao_box td input:checked').each(function () {
            ...
            var amountS = parseFloat($(this).parents("tr").find(".zhuihao_amount_sip").html());
            total += (amountS);
        });

        $('#zhuihao_num').text(count);
    ...
    window.Follow = {
        ...
        addAction: function () {
            //追号事件监听
            $(document).on("change", ".zhuihao_table .zhuihao_beishu", function () {
                //修改input旁边的价格
                var aCount = $(this).val();
                $(this).parents("tr").find(".zhuihao_amount_sip").html(aCount * Follow.config.price_amount);
                zhuihao_data_func();
            });
        }
    };
    ...
    //追号单选
    $(document).on('click', ".zhuihao_box td input[type=checkbox]", zhuihao_data_func);
