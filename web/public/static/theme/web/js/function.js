$(function () {
    ...
    window.lottery = {
        ...
        set_game_zhui_hao: function () {
            ...
            if (choosed_tr.length === 0) {
                ...
                $.get('/game/zhuihao?type=' + game.type + '&num=' + num + '&beiShu=' + beiShu + '&mode=' + mode + '&amount=' + amount, function (ret) {
                    window.Follow.config = {
                        price_amount: ret.data.price_amount,
                    };
                    console.log(window.Follow.config);

                    layer.open({
                        content: ret.data.html,
                        btn: ['确认追号', '取消'],
                        yes: function (index) {
                            lottery.zhuihao_sure(ret.data);
        ...
        // 确定追号
        zhuihao_sure: function (json_config) {
            //配置
            var data = [];
            $('.zhuihao_box td input:checked').each(function () {
                ...
                //获取选择的倍数
                var beiShu = $(this).parents("tr").find("input.zhuihao_beishu").val();
            ...
            if (data.length > 0) {
                ...
                $('#btnZhuiHao').die('click').css({
                    'font-size': '12px',
                    'line-height': '34px'
                }).text('追号 ' + $('#zhuihao_num').text() + ' 期，' + $('#zhuihao_total').text() + ' 元').live('click', lottery.zhuihao_cancel);
        ...
        user_fresh: function () {
            //用户刷新
            $.post("/user/fresh", {}, function (ret) {
                if (ret.code === 200) {
                    $("#show-username").html(ret.data.username);
                    $("#show-user-money").html(ret.data.coin);
                    $("#show-user-score").html(ret.data.score);
                }
            })
        },
        // 玩法切换
        ...
        set_play_Pl: function (value, flag) {
            ...
            if (flag) {
                ...
                try {
                    setTimeout(function () {
                        $(".nstSlider").attr("data-range_max", max_userFanDian * 10);
                        $(".nstSlider").nstSlider("set_range", 0, max_userFanDian * 10);
                        $(".nstSlider").nstSlider("set_position", fanDian_this * 10, fanDian_this * 10);
                        console.log(fanDian_this);
        ...
        game_add_code: function () {
            ...
            var fandian_value = window.fandianSelArr[currentFanDianValue];
            if (!fandian_value) {
        ...
        game_add_code_func: function (code, opt, real) {
            ...
            if ($.isArray(code)) {
                ...
                if (real === false) {
                    ...
                    $('#all-amount').text(all_amount.round(4));
                } else {
                    $.error_show = true;
                }
                return;
                ...
                if (
                    ...
                    for (var iii = 0; iii < weizhiAr.length; iii++) {
                        ...
                        var tr = $('<tr class="code">').data('code', code)
                            ...
                            .append($('<td>').data('value', amount).append(amount.round(4) + '元'))
                            .append($('<td>').append(code.beiShu + '倍'))
                            .append($('<td>').append(modeName[code.mode]))
                            .append($('<td>').append(parseFloat(code.bonusProp).round(4) + ' - ' + parseFloat(code.fanDian).round(1) + '%'))
        ...
        game_post_code: function () {
            ...
            loadpage.before(function () {
                ...
                var current = $.parseJSON($.ajax({
                    url: '/game/current?id=' + game.type,
                    type: 'post',
                    async: false,
                    cache: false,
                }).responseText);
        ...
        countdown: function (diffTime, actionNo) {
            ...
            if (diffTime <= 0) {
                ...
                //修整页面提示
                layer.open({
                    content: '请注意，当前期数已经结束投注，进行封单，请注意开奖结果',
                    time: 3
                });
        ...
        get_loss_gain: function (type, actionNo) {
            if (type && actionNo) {
                //弹窗
                $.post('/tip/loss_gain?id=' + type + '&actionNo=' + actionNo, {}, function (ret) {
                    var pageii = layer.open({
                        ...
                        shade: false,
                        btn: ["我知道了"],
                        title: ['上期亏盈提示', 'background-color: #FF4351; color:#fff;'],
                        style: 'position: fixed;display: block;background: #fff;right: 0;width: 10%;border: none;color: #000;bottom: 20%;',
                    });
                }, "JSON");
            }
        },
