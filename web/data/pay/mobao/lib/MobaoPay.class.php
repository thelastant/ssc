...
class MbPay
{
    ...
    {
        $sHtml = "<form id='pay_action_form' target='_blank' name='pay_action_form' action='" . $gateway . "' method='post'>";
        foreach ($data as $key => $val) {
            $sHtml .= "<input type='hidden' name='" . $key . "' value='" . $val . "'/>";
        }
        $sHtml .= "</form>";
        //$sHtml .= "<script type='text/javascript'>document.forms['pay_action_form'].submit();alert(1)</script>";
        return $sHtml;
    }
