...
//百家乐
exports.bjldx = function (bet, kj) {
    bet = bet.split(' ');
    kj = kj.split(',');
    z = parseInt(kj[0]) + parseInt(kj[1]);
    x = parseInt(kj[3]) + parseInt(kj[4]);
    if (z >= 10) z = z % 10;
    if (x >= 10) x = x % 10;
    if (z > x) {
        if (bet[0] == '庄') return 1;
...
exports.bdwQ32 = function (bet, kj) {
    kj = filterArray(kj.substr(0, 5).split(','));
    bet = bet.split(' ');
...
exports.bdwH32 = function (bet, kj) {
    kj = filterArray(kj.substr(4, 5).split(','));
    bet = bet.split(' ');
    if (bet.length < kj.length) {
        if (Sames(kj, bet) >= 2) return Combination(Sames(kj, bet), 2);
    } else if (bet.length >= kj.length) {
...
exports.bdw4x2m = function (bet, kj) {
    kj = filterArray(kj.substr(2, 7).split(','));
    bet = bet.split(' ');
    if (bet.length < kj.length) {
        if (Sames(kj, bet) >= 2) return Combination(Sames(kj, bet), 2);
    } else if (bet.length >= kj.length) {
...
exports.qwwfq3qw2x = function (bet, kj) {
    ...
    var bet0 = bet[0].split('');
    var bet1 = bet[1].split('');
    var bet2 = bet[2].split('');
    if (bet[1].indexOf(kj[1]) != -1) {
        if (bet[2].indexOf(kj[2]) != -1) {
            if (kj[0] < 5) {
                if (bet[0].indexOf('小') != -1) {
                    return 2;
                } else {
                    return 1;
                }
            } else {
                if (bet[0].indexOf('大') != -1) {
                    return 2;
                } else {
                    return 1;
                }
...
exports.qwwf4mqw3x = function (bet, kj) {
    kj = kj.substr(2, 7).split(',');
    bet = bet.split(',');
    var bet0 = bet[0].split('');
    var bet1 = bet[1].split('');
    var bet2 = bet[2].split('');
    var bet3 = bet[3].split('');
    if (bet[1].indexOf(kj[1]) != -1) {
        if (bet[2].indexOf(kj[2]) != -1) {
...
exports.gd11x5Q1 = function (bet, kj) {
    ...
    bet = bet.split(' ');
    for (var i = 0; i < bet.length; i++) {
        if (parseInt(kj[0]) == parseInt(bet[i])) {
            return 1;
            break;
        }
    }
...
exports.qwwfczw = function (bet, kj) {
    bet = bet.split(' ');
    var zs = "";
    kj = kj.split(',').sort(function compare(a, b) {
        return a - b;
    });
    for (var i = 0; i < bet.length; i++) {
...
exports.k32fx = function (bet, kj) {
    var betArr = bet.replace(/\*/g, "").replace(/\ /g, "").split('');
    var betLog = {};
    var data = kj.split(',');
    var dataLength = data.length;
    for (var i = 0, j = betArr.lenngth; i < j; i++) {
        var betNum = betArr[i];
        if (betNum === undefined) continue;
        if (!betLog.hasOwnProperty(betNum)) {
            var times = 0;
            betLog[betNum] = true;
            for (var _i = 0; _i < dataLength; _i++) {
                var dataNum = data[_i];
                if (dataNum === undefined) continue;
                if (dataNum === betNum) times = times + 1;
            }
            if (times >= 2) return 1;
        }
    }
...
exports.LTTSD = function (bet, kj) {
    var count = 0;
    bet = bet.replace(new RegExp("尾", "g"), '');
    kj = kj.split(',').map(function (v) {
        return v.split('')[1].split('').join(',')
    });
    if (kj.indexOf(bet) != -1) count = 1;
    return count;
}
