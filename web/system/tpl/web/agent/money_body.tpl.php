...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	...
	<tr>
		...
		<td>
			<a href="/agent/money?parentId=<?php echo $v['uid'];?>&fromTime=<?php echo date('Y-m-d H:i', $this->request_time_from);?>&toTime=<?php echo date('Y-m-d H:i', $this->request_time_to);?>" container="#agent-money-dom .body" data-ispage="true"  class="icon-download">下级</a>
			<?php if($v['uid'] != $this->user['uid'] && $v['parentId']) {?>
			  <a href="/agent/money?uid=<?php echo $v['uid'];?>&fromTime=<?php echo date('Y-m-d H:i', $this->request_time_from);?>&toTime=<?php echo date('Y-m-d H:i', $this->request_time_to);?>" container="#agent-money-dom .body" data-ispage="true"  class="icon-upload">上级</a>
			<?php }?>
		</td>
	</tr>
	<?php }?>
