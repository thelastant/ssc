...
if ($data) {
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        foreach ($data as $v) {
            ...
            ?>
            <tr>
                <td><a href="/bet/info?id=<?php echo $v['id']; ?>" title="投注信息"
                       target="layer-html"><?php echo $v['wjorderId']; ?></a></td>
                ...
                <td><?php echo $v['lotteryNo'] ? number_format($v['bonus'], 4, '.', '') : '0.00'; ?></td>
                ...
                <td style="display:none">
                    <?php if ($v['lotteryNo'] || $v['isDelete'] == 1 || $v['kjTime'] < $this->time) { ?>
                        --
                    <?php } else { ?>
                        <a href="javascript:;" data-id="<?php echo $v['id']; ?>" remove="false"
                           class="remove_single">撤单</a>
                    <?php } ?>
                </td>
            </tr>
        <?php } ?>
...
<script>
    seajs.use(["layer", "layerCss"], function () {
        $(document).on("click", "a[target=layer-html]", function (e) {
            e.preventDefault();
