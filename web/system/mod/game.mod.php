...
class mod_game extends mod
{
    ...
    public function index()
    {
        ...
        if (!$data) core::error("对不起，未找到该类别彩种，或者系统并未开通");
    ...
    public function wb_data_logs()
    {
        if ($this->post) {
            ...
            $this->display("pages/cp_open_logs", array(
    ...
    {
        $id = $this->request->request("id", 0, "intval");
        $limit = $this->request->request("limit", 20, "intval");
        if ($limit >= 20) {
            $limit = 20;
        }
        $types = core::lib('game')->get_types();
        if (!array_key_exists($id, $types)) {
            //非法操作
            $this->api_err_return("非法数据接口获取");
        }
        //查询开奖记录
        $map = array();
        $map["type"] = $types[$id]["id"];
        $data = $this->db->search("{$this->db_prefix}data", $map, "data,number,time", "ORDER BY `number` DESC", intval($limit));
    ...
    public function zhuihao()
    {
        ...
        $html .= '<table class="zhuihao_table"  width="100%" cellpadding="0" cellspacing="0" class="table table-bordered">';
        $html .= '<tr class="head">';
        ...
        foreach ($nos as $no) {
            ...
            $html .= "<td><input type='number' class='zhuihao_beishu' value='{$oldBeishu}'></td>";
            $html .= '<td>' . $mode . '</td>';
            $html .= '<td class="zhuihao_amount_sip">' . $oldAmount * $oldBeishu . '</td>';
    ...
    {
        $this->check_post();
        if (!array_key_exists('code', $_POST) || !array_key_exists('para', $_POST) || !is_array($_POST['code']) || !is_array($_POST['para'])) core::__403();
        $this->time = time();
        $codes = $_POST['code'];
        $para = $_POST['para'];
        $amount = 0;
...
//core::error('平台正在更新，暂停下注');
        ...
        ) core::error('投注失败，您投注的第<span class="label label-danger">' . $para['actionNo'] . '</span>期已过购买时间');
        // 查检每注的赔率是否正常
        $plays = $this->get_plays();
        $types = core::lib('game')->get_types();
        foreach ($codes as $key => $code) {
            $code['fanDian'] = round(floatval($code['fanDian']), 1);
            ...
            $bonusPropPlay = $this->get_play_bonus($played['id']);

            $bonusProp = $bonusPropPlay['bonusProp'];
            $bonusPropBase = $bonusPropPlay['bonusPropBase'];
            //mode
            $bonusProp *= ($code['mode'] / 2);
            $bonusPropBase *= ($code['mode'] / 2);
            ...
            if (!array_key_exists('bonusProp', $code)) {
                core::error("操作错误");
            }

            if ($code['bonusProp'] > $bonusProp + 1) core::error('提交奖金大于最大奖金，请重新投注');
            if ($code['bonusProp'] < $bonusPropBase) core::error('提交奖金小于最小奖金，请重新投注');

            if (!array_key_exists('fanDian', $code) || !core::lib('validate')->number_float($code['fanDian'], 1)) {
                core::__403();
            }
            // 获取不同模式的返点的
            $fandian_modes = array(
            ...
            if (!$fandian_this) {
                $this->api_err_return("系统配置返点错误");
            }
            if ($code['fanDian'] > $fandian_this) {
                $this->api_err_return("返点大于系统最大返点");
            }

            //计算奖金
            $tmpArv = ($played['bonusProp'] - $played['bonusPropBase']) / $this->config['fanDianMax'];
        ...
        try {
            foreach ($codes as $code) {
				...
				if($code['mode']==0.2){
					$code['bonusProp']=$code['bonusProp']*10;
				}else if($code['mode']==0.02){
					$code['bonusProp']=$code['bonusProp']*100;
				}else if($code['mode']==0.002){
					$code['bonusProp']=$code['bonusProp']*1000;
				}
                $amount = abs($code['actionNum'] * $code['mode'] * $code['beiShu']);
